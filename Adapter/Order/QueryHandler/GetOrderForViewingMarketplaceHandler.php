...
namespace ShoppyGo\MarketplaceBundle\Adapter\Order\QueryHandler;
...
class GetOrderForViewingMarketplaceHandler extends AbstractOrderHandler implements GetOrderForViewingHandlerInterface
{
    ...
    {
        $this->is_seller_order = $this->sellerOrderRepository->isSellerOrder(
            $query->getOrderId()
                ->getValue()
        );
    ...
    {
        $this->sellerOrderRepository = $orderRepository;
    }
    ...
    private function buildFakeCustomerObject(Order $order, OrderInvoiceAddressForViewing $invoiceAddress): Customer
    {
        ...
        $customer->email = '';
        $customer->id_lang = $cart->getAssociatedLanguage()
            ->getId()
        ;
    ...
    private function getLinkedOrders(Order $order): LinkedOrdersForViewing
    {
        ...
        foreach ($brothersData as $brotherItem) {
            ...
            $brothers[] = new LinkedOrderForViewing(
                $brotherItem->id, $orderState->name[$this->context->language->getId()], $totalAmount
            );
        }
    ...
    private function getOrderCustomer(
        ...
        return new OrderCustomerForViewing(
            ...
            $totalSpentSinceRegistration !== null ? $this->locale->formatPrice(
    ...
    private function getOrderPayments(Order $order): OrderPaymentsForViewing
    {
        ...
        // if order is a seller order, no chech Payment Mismatch
        // if order is a main order, check Payment Mismatch but not consider seller orders
        if ((count($payments) > 0) && ($this->is_seller_order === false)) {
//           $total_order_payments = $order->getOrdersTotalPaid();
//            $total_order_payments = (float)Db::getInstance()
//                ->getValue(
...
//                )
//            ;
            $total_order_payments = $this->sellerOrderRepository->getMainOrdersTotalPaid($order->id,
                $order->id_cart);
            $noPaymentMismatch = round($total_order_payments, 2) == round($order->getTotalPaid(), 2) ||
                ($currentState && $currentState->id == $this->configuration->getInt('PS_OS_CANCELED'));

            if (!$noPaymentMismatch) {
                $orderAmountToPay = $this->locale->formatPrice($total_order_payments, $currency->iso_code);
        ...
        foreach ($order->getOrderPaymentCollection() as $payment) {
            ...
            $invoiceNumber =
                $invoice ? $invoice->getInvoiceNumberFormatted($this->contextLanguageId, $order->id_shop) : null;
        ...
        return new OrderPaymentsForViewing(
            $orderPayments, $orderAmountToPay, $orderAmountPaid, $paymentMismatchOrders
        );
    }
